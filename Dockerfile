...
RUN git clone --depth 1 --branch skillspacexr https://github.com/hiram-labs/lms-directus.git \
    && cd lms-directus \
    && cp /tmp/.env .\
    && corepack enable \
...
RUN git clone --depth 1 --branch skillspacexr https://github.com/hiram-labs/lms-studio.git \
    && cd lms-studio \
    && cp /tmp/.env .\
    && npm ci \
    && npm run build \
...
RUN git clone --depth 1 --branch skillspacexr https://github.com/hiram-labs/lrs-core.git \
    && cd lrs-core \
    && cp /tmp/.env .\
    && npm ci \
    && npm run build-all \
...
RUN git clone --depth 1 --branch skillspacexr https://github.com/hiram-labs/lrs-xapi-service.git \
    && cd lrs-xapi-service \
    && cp /tmp/.env .\
    && npm ci \
    && npm run build \
